...
export async function GET(request: Request) {
  const id = new URL(request.url).searchParams.get("id"); const file = id ? speechService.file(id) : undefined;
  if (!file) return Response.json({ error: "Speech clip not found." }, { status: 404 });
  let size: number; try { size = (await stat(file.path)).size; } catch { return Response.json({ error: "The local audio file is missing." }, { status: 404 }); }
  const common = { "accept-ranges": "bytes", "content-type": file.record.mimeType, "cache-control": "private, max-age=3600" };
  const range = request.headers.get("range");
  if (!range) return stream(file.path, 0, size - 1, 200, { ...common, "content-length": String(size) });
  ...
  return stream(file.path, start, end, 206, { ...common, "content-length": String(end - start + 1), "content-range": `bytes ${start}-${end}/${size}` });
}
